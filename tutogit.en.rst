...
Needless to say, `I am using git constantly <https://bitbucket.org/lbesson/>`_, mainly when I work on Linux (Ubuntu™ 14.04) on a daily basis, and from time to time on Windows™.
...
Those three allow you to see the changed, deleted or added files:
...
`And that last one <https://bitbucket.org/lbesson/kaggle>`_ is also `here locally on my website (publis/kaggle) <publis/kaggle/>`_.
